...
- Most privileged operations (`rebootDevice`, `addToLockTaskMode`, `setAsPersistentHomeActivity`, `setApplicationHidden`, `setLockTaskFeatures`) gate on `dpm.isDeviceOwnerApp(...)` and throw if the app isn't the device owner — this is a real runtime precondition (the app must be provisioned as device owner via `adb shell dpm set-device-owner ...` or similar), not just defensive code.
...
- `setAsPersistentHomeActivity` controls a *different* problem than lock task mode: which activity Home resolves to, not what happens once an activity is running. Without it, pressing Home while locked shows Android's launcher-picker resolver whenever another HOME-capable app is installed, and picking one exits lock task mode entirely. It only sets a *preference* via `DevicePolicyManager.addPersistentPreferredActivity` — the host app must still declare its own HOME/DEFAULT intent-filter (typically via a config plugin) for the target activity to be Home-capable at all; this module has no visibility into that and can't do it on the app's behalf. It targets the current activity, so it must be called from whichever activity should become Home. Per Android's own docs, the persistent preference is designed to survive resets that would clear an ordinary (non-persistent) preferred-activity record — so, unlike `addToLockTaskMode`, there isn't a strong technical need to recall it on every resume. Still recommended to call it alongside `addToLockTaskMode()` on app-active resume anyway (cheap, idempotent, covers the initial call never having succeeded), just don't justify that with a false "doesn't survive Settings" claim.
- `setApplicationHidden` exists because `setAsPersistentHomeActivity` and lock task mode each only close one specific path to a competing launcher (Home resolution; Overview/Recents under the standard DPM-governed recents path) — an OEM launcher's own gesture-nav overlay can still reach its Recents/Overview UI outside that path entirely and jump to its own home screen from there. Hiding the competing package via `DevicePolicyManager.setApplicationHidden` closes every path to it at once (icon, Recents, Home resolution) instead of chasing each new escape route individually. Note `setApplicationHidden` returns a `Boolean` (not `void`, unlike the other DPM calls in this file) — the Kotlin wrapper must check it and throw on `false`, or a failed call would silently no-op exactly like the `addToLockTaskMode` bug fixed earlier. `packageName` is caller-supplied, not enumerated/discovered by this module — it has no way to know which other HOME-capable packages exist on a given device's fleet.
- `enableImmersiveMode`'s API 30+ path must use `WindowInsetsController.BEHAVIOR_SHOW_TRANSIENT_BARS_BY_SWIPE`, not `BEHAVIOR_DEFAULT` — `BEHAVIOR_DEFAULT` leaves the system bars visible indefinitely once revealed by a swipe (no auto-hide), while the transient-by-swipe behavior auto-hides them again after the swipe, matching the official immersive-mode guide's own example. Even with the right constant, this only narrows the reveal window: Android intentionally keeps some system-bar reveal gesture reachable on non-rooted devices as an accessibility/escape valve, so full suppression isn't achievable through this API regardless of the constant chosen.
